...
import {
  FiHome, FiHeart, FiBarChart2, FiSun, FiMoon,
...
const Navbar = () => {
  ...
  const { lastUpdated } = useSelector(s => s.weather);
  ...
  const navLinks = [
    ...
    { to: '/compare', icon: FiBarChart2, label: 'Compare' },
  ];
      ...
      <div className="glass-card mx-4 mt-3 rounded-2xl px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            ...
            >
              <WiDaySunny className="text-white" size={20} />
            </motion.div>
            <div>
              <span className="font-display font-bold text-white text-lg leading-none">SkyPulse</span>
              <div className="text-white/40 text-[10px] font-mono leading-none">WEATHER</div>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-2">
            {lastUpdated && (
              <div className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10">
                ...
                <span className="text-white/50 text-xs font-mono">Live</span>
              </div>
            )}
              ...
              <AnimatePresence>
                {userMenuOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-12 glass-card rounded-xl p-2 min-w-[160px]"
                  >
                    {isAuthenticated ? (
                      <>
                        <button className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-white/70 hover:text-white hover:bg-white/10 text-sm transition-all">
                          <FiSettings size={14} /> Settings
                        </button>
                        <button
                          onClick={() => { dispatch(logout()); setUserMenuOpen(false); }}
                          className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-red-400 hover:bg-red-500/10 text-sm transition-all"
                        >
                      ...
                      <>
                        <Link to="/login" onClick={() => setUserMenuOpen(false)}
                          className="flex items-center gap-2 px-3 py-2 rounded-lg text-white/70 hover:text-white hover:bg-white/10 text-sm transition-all">
              ...
              <div className="flex flex-col gap-1 pt-3 border-t border-white/10 mt-3">
                {navLinks.map(link => (
                  <div key={link.to} onClick={() => setMobileOpen(false)}>
                    <NavLink {...link} active={location.pathname === link.to} />
                  </div>
                ))}
              </div>
            </motion.div>
